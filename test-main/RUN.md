...
## Интерактивный просмотр фронта

`output/pareto_front.html` — все 60 максимальных элементов фронта с
показателями, сортировкой и фильтрами. Открывается двойным щелчком в любом
браузере, запускать ничего не нужно:

```bash
xdg-open output/pareto_front.html     # Linux
open output/pareto_front.html         # macOS
start output\pareto_front.html        # Windows
```

Файл самодостаточен: данные встроены внутрь, динамической загрузки нет,
интернет не требуется. Единственная внешняя ссылка — веб-шрифты; без сети
страница отрисуется системным шрифтом и останется полностью читаемой.
Ключи, регистрация и оплата не нужны.
